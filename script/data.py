...
class MyData:
    ...
    def get(self, qty, array=True):
        if qty in self.qty_name and qty in self.dataset:
    ...
    def filt(self, qty, val, cond=None):
        """
        filt qty0 data according to qty = val or cond(qty, val)=True
        """
        sub_data = MyData(qty_name=self.qty_name)
        
        if cond is None:
            for i, val1 in enumerate(self.get(qty)):
                if val1 == val:
                    for qtyi in self.qty_name:
                        sub_data.append(qtyi, self.get(qtyi)[i])
        else:
            for i, val1 in enumerate(self.get(qty)):
                if cond(val1, val):
                    for qtyi in self.qty_name:
    ...
    def load(self, filename, append=True, mod="append", key_qtys = [], output=False):
        """
        ...
        try:
            ...
            if mod == "append":
                ...
                self.update(data_temp, key_qtys = key_qtys)
            elif mod == "replace":
                self.qty_name = []
            ...
            if output:
                max_length, min_length = 0, 9999
                for k, v in self.dataset.items():
                    if isinstance(v, list):
                        max_length = max(max_length, len(v))
                        min_length = min(min_length, len(v))
                    
                print("load complete, max size: ", max_length) #, ", min size: ", min_length )
        except:
    ...
    def update(self, other_data, key_qtys):
        ### update val if key qtys are the same
        max_length = 0 
        for key, val in self.dataset.items():
            if isinstance(val, list):
                max_length = max(max_length, len(val))
        
        index_of_updates = [True] * max_length
        index_in_other_data = [None] * max_length
        for i in range(max_length):
            for key, val in self.dataset.items():
                if key in key_qtys and key in other_data:
                    if not isinstance(val, list):
                        continue 
                    if (val[i] not in other_data[key]):
                        index_of_updates[i] = False
                    else:
                        index_in_other_data[i] = other_data[key].index(val[i])
        for i in range(max_length):
            if index_of_updates[i]:
                for key, val in self.dataset.items():
                    val[i] = other_data[key][index_in_other_data[i]]
        return
